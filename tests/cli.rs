...
use std::{fs, path::Path, process::Command as StdCommand, sync::OnceLock};
...
fn setup_repo_with_non_linear_version_tags() -> Result<TempDir, Box<dyn std::error::Error>> {
    let temp = tempdir()?;
    let repo = temp.path();
...
fn mask_oids(input: &str) -> String {
    ...
    while i < chars.len() {
        ...
        while i + len < chars.len()
            && chars[i + len].is_ascii_hexdigit()
            && (!chars[i + len].is_ascii_alphabetic() || chars[i + len].is_ascii_lowercase())
        {
            len += 1;
        }
        ...
        if len >= 7
            ...
            && (i + len == chars.len() || !chars[i + len].is_ascii_hexdigit())
        {
            result.push_str("<OID>");
            i += len;
        } else {
...
fn mask_dates(input: &str) -> String {
    static DATE_REGEX: OnceLock<Regex> = OnceLock::new();
    let date_regex =
        DATE_REGEX.get_or_init(|| Regex::new(r"\([0-9]{4}-[0-9]{2}-[0-9]{2}\)").unwrap());
    date_regex.replace_all(input, "(<DATE>)").to_string()
}

fn sanitize_output(input: &str) -> String {
    let masked_oids = mask_oids(input);
    mask_dates(&masked_oids)
}
